...
const range = document.querySelector('.size-range') // 선 두께 조절
...
function setColor(e) {
  ...
  if (!selectedButton.classList.contains('color-btn')) {
    return; // 클릭된 요소가 color-btn이 아닌 경우 처리하지 않음
  }
...
function setPencilMode() {
  ...
  // 활성화 클래스 설정
  pencil.classList.add('draw-tool-btn-active');
  eraser.classList.remove('draw-tool-btn-active');
...
function setEraserMode() {
  ...
  // 활성화 클래스 설정
  pencil.classList.remove('draw-tool-btn-active');
  eraser.classList.add('draw-tool-btn-active');
...
.then(stream => {
  ...
  btnToggleAudio.addEventListener('click', () => {
    ...
    if(audioTracks[0].enabled) {
      btnToggleAudio.textContent = '음소거'
    } else {
      btnToggleAudio.textContent = '음소거 해제'
    }
  })
  ...
  btnToggleVideo.addEventListener('click', () => {
    ...
    if(videoTracks[0].enabled) {
      btnToggleVideo.textContent = '비디오 중지'
    } else {
      btnToggleVideo.textContent = '비디오 시작'
    }
  })
})
.catch((error) => {
